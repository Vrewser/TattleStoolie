...
from models.user import Reporter


class ReporterLoginFrame(ctk.CTkFrame):
    """Reporter-only login frame."""
    ...
    def __init__(self, parent, app):
        ...
        register_btn = ctk.CTkButton(
            ...
            width=120, height=36,
            corner_radius=6,
            command=lambda: app.show_frame("ReporterRegisterFrame")
        )
    ...
    def login(self):
        ...
        # Verify user is reporter (not admin)
        role = row.get('role', '').lower()
        if role not in ('reporter', 'viewer'):
            messagebox.showerror("Error", "Reporter credentials required.")
            return
        ...
        # Store current_user and navigate to reporter interface
