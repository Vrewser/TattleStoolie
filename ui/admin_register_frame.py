...
class AdminRegisterFrame(ctk.CTkFrame):
    """Admin-only registration frame. Note: Admins should be seeded by super-admin only."""
    ...
    def __init__(self, parent, app):
        ...
        back_to_login_btn = ctk.CTkButton(
            ...
            width=100, height=36,
            corner_radius=6,
            command=lambda: app.show_frame("AdminLoginFrame")
        )
    ...
    def register(self):
        ...
        # Create admin user
        ok = self.app.db.create_user(u, e, p, role="admin")
        if not ok:
            messagebox.showerror("Error", "Username already taken.")
            return

        messagebox.showinfo("Success", "Admin account created. Please log in.")
        self.app.show_frame("AdminLoginFrame")
